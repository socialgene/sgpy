    ...
    with tempfile.TemporaryDirectory() as fp:
        ...
        for fname in fnames:
            with open({i.stem: i for i in contents}[fname], "r") as h:
                holdit[fname] = h.readlines()
        log.info(expected_file_contents)
        log.info([holdit["amrfinder_hmm_source"][0]])
        assert expected_file_contents == [holdit["amrfinder_hmm_source"][-0]]
